...
export default function DashboardLayout() {
    ...
    const navItems = [
        ...
        { path: '/dashboard/garage', icon: Car, label: 'გარაჟი' },
    ...
    return (
        <div className="min-h-screen bg-surface pb-24 sm:pb-0">
            {/* ═══ Navbar ═══ */}
            <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-white/90 backdrop-blur-2xl shadow-card' : 'bg-white/60 backdrop-blur-xl'
                }`}>
                <div className="section-container">
                    <div className="flex items-center justify-between h-14">
                        {/* Logo */}
                        <Link to="/dashboard" className="flex items-center gap-2.5 group">
                            <img src="/logo-carmed.png" alt="CarMed" className="h-7 w-auto" />
                        </Link>
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <div className="relative" ref={menuRef}>
                                <button onClick={() => setMenuOpen(!menuOpen)}
                                    className={`flex items-center gap-2 pl-1.5 pr-2 py-1 rounded-[12px] transition-all ${menuOpen ? 'bg-surface-50' : 'hover:bg-surface-50/60'
                                        }`}>
                                ...
                                {menuOpen && (
                                    <div className="absolute right-0 top-full mt-1.5 w-52 card-elevated p-1.5 animate-scale-in origin-top-right">
            ...
            {/* ═══ Mobile Bottom Nav ═══ */}
            <nav className="sm:hidden fixed bottom-0 left-0 right-0 z-50 px-4 pb-3 safe-area-bottom">
                <div className="bg-white/90 backdrop-blur-2xl rounded-[18px] shadow-float border border-white/60 
                    mx-auto max-w-[360px]">
                    <div className="flex items-center justify-around h-14 px-1">
                        {navItems.map(({ path, icon: Icon, label, end }) => (
                            <NavLink key={path} to={path} end={end}
                                className={({ isActive }) =>
                                    `flex flex-col items-center gap-0.5 px-4 py-1.5 rounded-[12px] transition-all duration-200 ${isActive ? 'text-emerald-700' : 'text-ink-faint'
                                    }`
                                }>
                                {({ isActive }) => (
                                    <>
                        ...
                        <NavLink to="/dashboard/orders/new"
                            ...
                            <div className="w-9 h-9 rounded-[10px] btn-dark flex items-center justify-center -mt-2">
